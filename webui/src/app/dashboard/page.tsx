...
export default function Dashboard() {
  ...
  const filteredRepos = useCallback(() => {
    ...
    if (filter === "codeSmells")
      repos = repos.filter((r) => r.metrics.codeSmells > 0);
    if (filter === "vulnerabilities")
      repos = repos.filter((r) => r.metrics.vulnerabilities > 0);
    ...
    repos.sort((a, b) => {
      switch (sortBy) {
        ...
        case "bugs":
          return b.metrics.bugs - a.metrics.bugs;
        case "smells":
          return b.metrics.codeSmells - a.metrics.codeSmells;
        case "vulns":
          return b.metrics.vulnerabilities - a.metrics.vulnerabilities;
        case "ncloc":
          return b.metrics.ncloc - a.metrics.ncloc;
        default:
          return 0;
      }
    });
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-2">
            ...
            />
            <FilterButton
              active={filter === "codeSmells"}
              onClick={() => setFilter("codeSmells")}
              icon={<Wind className="w-3.5 h-3.5" />}
              label="Smells"
              count={totals.codeSmells}
            />
            <FilterButton
              active={filter === "vulnerabilities"}
          ...
          <div className="flex gap-1 border border-border rounded-lg p-0.5">
            <button
              ...
              className={`p-1.5 rounded ${view === "grid" ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setView("list")}
              className={`p-1.5 rounded ${view === "list" ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              ...
              >
                ...
                <div className="flex gap-4 text-sm">
                  <MetricPill
                    ...
                    value={repo.metrics.bugs}
                    color="text-warning"
                  />
                  <MetricPill
                    icon={<Wind className="w-3 h-3" />}
                    value={repo.metrics.codeSmells}
                    color="text-info"
                  />
                  <MetricPill
                    icon={<ShieldAlert className="w-3 h-3" />}
                    value={repo.metrics.vulnerabilities}
                    color="text-danger"
                  />
                </div>
              ...
              <tbody>
                {repos.map((repo) => (
                  ...
                  >
                    ...
                    <td
                      className={`text-center px-3 py-3 tabular-nums ${repo.metrics.bugs > 0 ? "text-warning" : "text-muted-foreground"}`}
                    >
                      {repo.metrics.bugs}
                    </td>
                    <td
                      className={`text-center px-3 py-3 tabular-nums ${repo.metrics.codeSmells > 0 ? "text-info" : "text-muted-foreground"}`}
                    >
                      {repo.metrics.codeSmells}
                    </td>
                    <td
                      className={`text-center px-3 py-3 tabular-nums ${repo.metrics.vulnerabilities > 0 ? "text-danger" : "text-muted-foreground"}`}
                    >
...
function MetricPill({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 ${value > 0 ? color : "text-muted-foreground"}`}
    >
      {icon}
      {value}
    </span>
...
function FilterButton({
  ...
  return (
    ...
    >
      ...
      {count !== undefined && (
        <span
          className={`ml-0.5 ${active ? "opacity-80" : "text-muted-foreground"}`}
        >
          {count}
        </span>
      )}
