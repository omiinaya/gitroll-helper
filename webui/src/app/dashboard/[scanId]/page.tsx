...
export default function RepoDetail() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (res.ok) {
        ...
        setData(d);
        if (d.metrics.bugs === 0 && d.metrics.codeSmells > 0)
          setTab("codeSmells");
        if (
          d.metrics.bugs === 0 &&
          d.metrics.codeSmells === 0 &&
          d.metrics.vulnerabilities > 0
        )
  ...
  const tabCounts = {
    ...
    codeSmells: data.metrics.codeSmells,
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          <MetricCard
            label="Quality Gate"
            value={data.qualityGatePassed ? "PASS" : "FAIL"}
            pass={data.qualityGatePassed}
          />
          <MetricCard
            label="Bugs"
            value={data.metrics.bugs}
            warn={data.metrics.bugs > 0}
          />
          <MetricCard
            label="Code Smells"
            value={data.metrics.codeSmells}
            warn={data.metrics.codeSmells > 0}
          />
          <MetricCard
            label="Vulnerabilities"
            value={data.metrics.vulnerabilities}
            danger={data.metrics.vulnerabilities > 0}
          />
          <MetricCard
            label="Reliability"
            value={`${data.metrics.reliabilityRating}/5`}
          />
          <MetricCard
            label="Security"
            value={`${data.metrics.securityRating}/5`}
          />
          <MetricCard
            label="Lines"
            value={data.metrics.ncloc.toLocaleString()}
          />
        </div>
        ...
        {data.hasDetails ? (
          <div className="space-y-4">
            ...
            <div className="flex gap-1 border-b border-border">
              <TabButton
                ...
                label="Bugs"
                count={tabCounts.bugs}
                color="text-warning"
              />
              <TabButton
                active={tab === "codeSmells"}
                onClick={() => setTab("codeSmells")}
                icon={<Wind className="w-4 h-4" />}
                label="Code Smells"
                count={tabCounts.codeSmells}
                color="text-info"
              />
              <TabButton
                ...
                label="Vulnerabilities"
                count={tabCounts.vulnerabilities}
                color="text-danger"
              />
            </div>
          ...
          <div className="rounded-xl border border-border bg-card p-8 text-center space-y-3">
            <p className="text-muted-foreground">
              Detailed issue breakdown is not available for this scan (older
              scan data).
            </p>
            <p className="text-sm text-muted-foreground">
              The summary metrics above are still accurate.
            </p>
          </div>
        )}
      </main>
...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span
      className={`shrink-0 text-xs px-2 py-0.5 rounded font-medium ${styles[severity] ?? styles.INFO}`}
    >
      {severity}
    </span>
  );
...
function TabButton({
  ...
  return (
    ...
    >
      ...
      {label}
      <span
        className={`text-xs tabular-nums ${active ? "" : "text-muted-foreground"}`}
      >
        {count}
      </span>
    </button>
